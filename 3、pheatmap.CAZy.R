...
#输入分组
metadata = read.table (file = "metadataCAZy.txt", row.names = 1, header = T, sep = "\t")


CAZy_abundance = read.table (file = "diff_CAZy.txt", sep = "\t", row.names = 1, header = T)
CAZy_abundance <- CAZy_abundance[which(rowSums(CAZy_abundance) > 0),]
CAZy_abundance <- CAZy_abundance[,rownames(metadata)]
CAZy_abundance <-as.data.frame( t(CAZy_abundance))
...
metabolomic <- metabolomic[rownames(CAZy_abundance),]


res <-  corr.test(metabolomic,CAZy_abundance,use='pairwise',method='spearman',adjust='BH',alpha=0.05)
names(res)#查看列表每个项目
...
colnames(phenotyperlong) <- c("species","CAZy","Correlation")
# View(phenotyperlong)

phenotypeplong <- melt(phenotypep, id.vars = colnames(phenotypep))
colnames(phenotypeplong) <- c("species","CAZy","pvalue")
# View(phenotypeplong)


phenotypepadjlong <- melt(phenotypepadj, id.vars = colnames(phenotypepadj))
colnames(phenotypepadjlong) <- c("species","CAZy","padjust")


phenotypestatisticlong <- melt(statistic, id.vars = colnames(statistic))
colnames(phenotypestatisticlong) <- c("species","CAZy","statistic")


hebingphenotyperp <- merge(phenotyperlong, phenotypeplong , by = c("species","CAZy"))
hebingphenotyperpj <- merge(hebingphenotyperp,phenotypepadjlong, by = c("species","CAZy"))
hebingphenotyperpj <- merge(hebingphenotyperpj,phenotypestatisticlong, by = c("species","CAZy"))
# View(hebingphenotyperp)


library(openxlsx)
write.xlsx(hebingphenotyperpj, paste0("CAZy/species_CAZy_corr_un.all.xlsx"),
           append = FALSE,
           colNames = TRUE,
           rowNames = FALSE)
write.table(hebingphenotyperpj,file = "CAZy/species_CAZy_corr_un.all.txt",sep = "\t",quote = F,row.names = F)
...
#保存文件（筛选的）
write.xlsx(data_merge_filtered, paste0("CAZy/species_CAZy_corr_filter.all.xlsx"),
           append = FALSE,
           colNames = TRUE,
           rowNames = FALSE)

write.table(data_merge_filtered,file = "CAZy/species_CAZy_corr_filter.all.txt",sep = "\t",quote = F,row.names = F)
...
write.table(phenotyper, file=paste0("CAZy/heatmapCAZy.cor.txt"), quote = F, sep="\t", eol = "\n", na = "NA", dec = ".",row.names = T, col.names = T)
write.table(phenotypepadj, file=paste0("CAZy/heatmapCAZy.padj.txt"), quote = F, sep="\t", eol = "\n", na = "NA", dec = ".",row.names = T, col.names = T)
...
plotmat = read.table (file = "CAZy/heatmapCAZy.cor.txt", row.names = 1, header = T, sep = "\t")
stars = read.table (file = "CAZy/heatmapCAZy.padj.txt", row.names = 1, header = T, sep = "\t")

rows_to_delete <- c("Cronobacter_sakazakii",
...
annotation = as.data.frame(read.table(paste0("pheatmap.CAZy.txt"), header=T, row.names=1, sep="\t", comment.char="", stringsAsFactors = F,quot=""))
...
# Anno_col <-c("#8DD3C7", "#FFFFB3", "#BEBADA", "#FB8072", "#80B1D3", "#FDB462", "#B3DE69", "#FCCDE5")
# names(Anno_col) <- unique(annotation$Anno)
# 
...
#设置family颜色
getPalette_family = colorRampPalette(brewer.pal(27, "Set2"))
Family_col <- getPalette_family(27)#Family_col <- getPalette_family(10)
...
spe <- read.table("CAZy/species_order.txt", header = T)
PA <- read.table("CAZy/CAZy_order.txt", header = T)
column_order <- PA$x
row_order <- spe$x
...
pdf("CAZy/CAZy_species2.pdf",  width=15, height=20)
p
dev.off()


png("CAZy/CAZy_species2.png",  width=800, height=1000,units = "px")
p
dev.off()

write.table(names(column_order(p)),file = "CAZy/CAZy_order.txt",quote = F,row.names = F)
write.table(names(row_order(p)),file = "CAZy/species_order.txt",quote = F,row.names = F)
...
library(openxlsx)
write.xlsx(hebingphenotyperpj, paste0("CAZy/species_SCFA_corr_un.all.xlsx"),
           append = FALSE)
write.table(hebingphenotyperpj,file = "CAZy/species_SCFA_corr_un.all.txt",sep = "\t",quote = F,row.names = F)
...
#保存文件（筛选的）
write.xlsx(data_merge_filtered, paste0("CAZy/species_SCFA_corr_filter.all.xlsx"),
           append = FALSE)

write.table(data_merge_filtered,file = "CAZy/species_SCFA_corr_filter.all.txt",sep = "\t",quote = F,row.names = F)
...
species_SCFA = read.table (file = "CAZy/species_SCFA_corr_un.all.txt",  header = T, sep = "\t")
library("tidyr")
library(reshape2)
...
sc <- read.table("CAZy/species_order.txt", header = T)
spe <-  read.table("CAZy/species_order.txt", header = T)
row_order<- spe$x
column_order<- sc
...
pdf("CAZy/specie_SCFA.2.pdf",  width=8, height=15)
p1
dev.off()


png("CAZy/specie_SCFA.2.png",  width=900, height=1300,units = "px")
p1
dev.off()

write.table(names(row_order(p1)),file = "CAZy/SCFA_order.txt",quote = F,row.names = F)
...
#输入pathway
pathway = as.data.frame(t(read.table("diff_CAZy.txt",header = T,row.names = 1, sep = "\t", comment.char="",check.names=F,quot="", stringsAsFactors = F)))
...
#class(res)
phenotyperlong <- melt(phenotyper, id.vars = colnames(phenotyper))
colnames(phenotyperlong) <- c("CAZy","SCFA","Correlation")
# View(phenotyperlong)

phenotypeplong <- melt(phenotypep, id.vars = colnames(phenotypep))
colnames(phenotypeplong) <- c("CAZy","SCFA","pvalue")
# View(phenotypeplong)


phenotypepadjlong <- melt(phenotypepadj, id.vars = colnames(phenotypepadj))
colnames(phenotypepadjlong) <- c("CAZy","SCFA","padjust")


phenotypestatisticlong <- melt(statistic, id.vars = colnames(statistic))
colnames(phenotypestatisticlong) <- c("CAZy","SCFA","statistic")


hebingphenotyperp <- merge(phenotyperlong, phenotypeplong , by = c("CAZy","SCFA"))
hebingphenotyperpj <- merge(hebingphenotyperp,phenotypepadjlong, by = c("CAZy","SCFA"))
hebingphenotyperpj <- merge(hebingphenotyperpj,phenotypestatisticlong, by = c("CAZy","SCFA"))
# View(hebingphenotyperp)


library(openxlsx)
write.xlsx(hebingphenotyperpj, paste0("CAZy/CAZy_SCFA_corr_un.all.xlsx"))
write.table(hebingphenotyperpj,file = "CAZy/CAZy_SCFA_corr_un.all.txt",sep = "\t",quote = F,row.names = F)
...
#保存文件（筛选的）
write.xlsx(data_merge_filtered, paste0("CAZy/CAZy_SCFA_corr_filter.all.xlsx"))

write.table(data_merge_filtered,file = "CAZy/CAZy_SCFA_corr_filter.all.txt",sep = "\t",quote = F,row.names = F)


##########################################画图

pathway_SCFA = read.table (file = "CAZy/CAZy_SCFA_corr_un.all.txt",  header = T, sep = "\t")
library("tidyr")
library(reshape2)
#纵轴(画图为横轴)  ~ 横轴 , value.var = "pvalues"，中间填充的值，如果缺失则为NA
#acast为将“纵轴设置为行名，而dcast行名为1,2,3,4
...
pathway_SCFA_p   <- acast(pathway_SCFA, SCFA~CAZy , value.var = "pvalue")
pathway_SCFA_p <- as.data.frame(t(pathway_SCFA_p))
pathway_SCFA_cor <- acast(pathway_SCFA, CAZy~SCFA , value.var = "Correlation")
...
PA <- read.table("CAZy/CAZy_order.txt", header = T,sep = "\t")
SCFA <- read.table("CAZy/SCFA_order.txt", header = T,sep = "\t")
row_order<- SCFA$x
column_order<- PA$x
...
pdf("CAZy/CAZy_SCFA.2.pdf",  width=5, height=3)
p2
dev.off()


png("CAZy/CAZy_SCFA.2.png",  width=300, height=300,units = "px")
p2
dev.off()
